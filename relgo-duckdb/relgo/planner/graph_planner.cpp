...
JavaVM *CreateJavaVM(const std::string &jvm_options) {
	...
	if (status == JNI_OK) {
		...
		std::cerr << "Error, create java virtual machine failed. return JNI_CODE (" << status << ")" << std::endl;
	}
...
JavaVM *GetJavaVM(const std::string jvm_options = "") {
	if (_jvm == NULL) {
		...
		std::cout << "Found " << nVMs << " VMs existing in this process." << std::endl;
		JavaVM **buffer = new JavaVM *[nVMs];
		JNI_GetCreatedJavaVMs(buffer, nVMs, &nVMs); // 2. get the data
		for (auto i = 0; i < nVMs; ++i) {
			if (buffer[i] != NULL) {
				_jvm = buffer[i];
				std::cout << "Found index " << i << " VM non null " << reinterpret_cast<jlong>(_jvm) << std::endl;
				return _jvm;
			}
		}
...
JNIEnvMark::JNIEnvMark(const std::string &jvm_options) : _env(NULL) {
	...
	int status = GetJavaVM(jvm_options)->AttachCurrentThread(reinterpret_cast<void **>(&_env), nullptr);
	if (status != JNI_OK) {
	...
	if (dir != nullptr) {
		...
		while ((entry = readdir(dir)) != nullptr) {
			...
			if (name != "." && name != "..") {
				...
				struct stat st;
				if (stat(full_path.c_str(), &st) == 0 && !(st.st_mode & S_IFDIR)) {
...
void iterate_over_director(const std::string &dir_or_path, std::vector<std::string> &output_paths) {
	...
	struct stat st;
	if (stat(dir_or_path.c_str(), &st) == 0 && (st.st_mode & S_IFDIR)) {
...
std::string GraphPlannerWrapper::generate_jvm_options(const std::string java_path, const std::string &jna_path,
...
Plan compilePlanJNI(jclass graph_planner_clz_, jmethodID graph_planner_method_id_, JNIEnv *env,
                    const std::string &compiler_config_path, const std::string &cypher_query_string,
                    const std::string &graph_schema_yaml, const std::string &graph_statistic_json) {
	jni::GetJavaVM()->AttachCurrentThread(reinterpret_cast<void **>(&env), nullptr);
	Plan plan;
	...
	jobject jni_plan = (jobject)env->CallStaticObjectMethod(graph_planner_clz_, graph_planner_method_id_, param1,
	...
	jmethodID get_error_code = env->GetMethodID(env->GetObjectClass(jni_plan), "getErrorCode", "()Ljava/lang/String;");
	...
	if (plan.error_code != "OK") {
		jmethodID get_full_msg =
		    env->GetMethodID(env->GetObjectClass(jni_plan), "getFullMessage", "()Ljava/lang/String;");
	...
	jmethodID method1 = env->GetMethodID(env->GetObjectClass(jni_plan), "getPhysicalBytes", "()[B");
	jmethodID method2 = env->GetMethodID(env->GetObjectClass(jni_plan), "getResultSchemaYaml", "()Ljava/lang/String;");
...
void write_query_to_pipe(const std::string &path, const std::string &query_str) {
...
void write_query_to_file(const std::string &path, const std::string &query_str) {
...
physical::PhysicalPlan compilePlanSubprocess(const std::string &class_path, const std::string &jna_path,
                                             const std::string &graph_schema_yaml,
                                             const std::string &graph_statistic_json,
                                             const std::string &compiler_config_path,
                                             const std::string &cypher_query_string) {
	physical::PhysicalPlan physical_plan;
	auto random_prefix = std::to_string(std::chrono::system_clock::now().time_since_epoch().count());
	...
	std::cout << "dst_query_path: " << dst_query_path << " dst_output_file: " << dst_output_file << std::endl;
	...
	if (pid == 0) {
		const char *const command_string_array[] = {"java",
		                                            ...
		                                            NULL};
		execvp(command_string_array[0], const_cast<char *const *>(command_string_array));
	} else if (pid < 0) {
		...
		physical_plan = readPhysicalPlan(std::string(stored_buffer.begin(), stored_buffer.end()));
		close(fd_from_java);
 ...
 */
Plan GraphPlannerWrapper::CompilePlan(const std::string &compiler_config_path, const std::string &cypher_query_string,
                                      const std::string &graph_schema_yaml, const std::string &graph_statistic_json) {
#if (GRAPH_PLANNER_JNI_INVOKER)
	return compilePlanJNI(graph_planner_clz_, graph_planner_method_id_, jni_wrapper_.env(), compiler_config_path,
	                      cypher_query_string, graph_schema_yaml, graph_statistic_json);
#else
	return compilePlanSubprocess(class_path_, jna_path_, graph_schema_yaml_, graph_statistic_json_,
	                             compiler_config_path, cypher_query_string);
#endif
}
