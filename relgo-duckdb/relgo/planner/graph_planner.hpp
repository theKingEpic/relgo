...
public:
	static constexpr const char *kGraphPlannerClass = "com/alibaba/graphscope/sdk/PlanUtils";
	static constexpr const char *GRAPH_PLANNER_FULL_NAME = "com.alibaba.graphscope.sdk.PlanUtils";
	...
	GraphPlannerWrapper(const std::string java_path, const std::string &jna_path,
...
#if (GRAPH_PLANNER_JNI_INVOKER)
	    : jni_wrapper_(generate_jvm_options(java_path, jna_path, graph_schema_yaml, graph_statistic_json)) {
		...
		jmethodID j_method_id = jni_wrapper_.env()->GetStaticMethodID(graph_planner_clz_, kGraphPlannerMethod,
		                                                              kGraphPlannerMethodSignature);
		if (j_method_id == NULL) {
...
#else
	    : jna_path_("-Djna.library.path=" + jna_path), graph_schema_yaml_("-Dgraph.schema=" + graph_schema_yaml),
	 ...
	 */
	Plan CompilePlan(const std::string &compiler_config_path, const std::string &cypher_query_string,
	                 const std::string &graph_schema_yaml, const std::string &graph_statistic_json);

private:
	std::string generate_jvm_options(const std::string java_path, const std::string &jna_path,
	                                 const std::string &graph_schema_yaml, const std::string &graph_statistic_json);
