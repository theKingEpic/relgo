...
#include "indexed_join_condition.hpp"
...
public:
  SIPHTScanState(TupleDataCollection &collection, idx_t chunk_idx_from,
                 idx_t chunk_idx_to,
...
public:
  ...
  private:
    ...
    //! Scan the hashtable for matches of the specified keys, setting the
    //! found_match[] array to true or false for every tuple
    void ScanKeyMatches(DataChunk &keys);
    template <bool MATCH>
    void NextSemiOrAntiJoin(DataChunk &keys, DataChunk &left,
                            DataChunk &result);

    void ConstructMarkJoinResult(DataChunk &join_keys, DataChunk &child,
                                 DataChunk &result);
  ...
  public:
    ...
    void GatherResult(Vector &result, const SelectionVector &result_vector,
                      const SelectionVector &sel_vector, const idx_t count,
                      const idx_t col_idx);
    void GatherResult(Vector &result, const SelectionVector &sel_vector,
...
public:
  SIPHashTable(BufferManager &buffer_manager,
               const vector<IndexedJoinCondition> &conditions,
               vector<LogicalType> build_types, JoinType type);
  SIPHashTable(BufferManager &buffer_manager,
               const vector<IndexedJoinCondition> &conditions,
               const vector<IndexedJoinCondition> &other_conditions,
               vector<LogicalType> build_types, JoinType type);
  ~SIPHashTable();
  ...
  //! Combines the partitions in sink_collection into data_collection, as if it
  //! were not partitioned
  void Unpartition();
  //! Initialize the pointer table for the probe
  void InitializePointerTable();
  //! Finalize the build of the HT, constructing the actual hash table and
  //! making the HT ready for probing. Finalize must be called before any call
  //! to Probe, and after Finalize is called Build should no longer be ever
  //! called.
  ...
  void ScanFullOuter(SIPHTScanState &state, Vector &addresses,
                     DataChunk &result);
  ...
  idx_t Count() const { return data_collection->Count(); }
  idx_t SizeInBytes() const { return data_collection->SizeInBytes(); }

  PartitionedTupleData &GetSinkCollection() { return *sink_collection; }

  TupleDataCollection &GetDataCollection() { return *data_collection; }
  ...
  bool has_null;
  //! Bitmask for getting relevant bits from the hashes to determine the
  //! position
  uint64_t bitmask;

  struct {
    mutex mj_lock;
    //! The types of the duplicate eliminated columns, only used in correlated
    //! MARK JOIN for flattening ANY()/ALL() expressions
...
private:
  ...
  InitializeScanStructure(DataChunk &keys, const SelectionVector *&current_sel);
  void Hash(DataChunk &keys, const SelectionVector &sel, idx_t count,
            Vector &hashes);

  //! Apply a bitmask to the hashes
  void ApplyBitmask(Vector &hashes, idx_t count);
  void ApplyBitmask(Vector &hashes, const SelectionVector &sel, idx_t count,
                    Vector &pointers);
  void ApplyKeymask(Vector &hashes, const SelectionVector &sel, idx_t count,
                    Vector &pointers);

  void FillBitmaskWithAList(
      Vector &key_vector, idx_t count,
      relgo::GraphIndexInfo<int64_t *, SelectionVector> &rai_info,
      relgo::PushDownStatistics &rai_stat);
  void FillBitmaskWithoutAList(
      Vector &key_vector, idx_t count,
      relgo::GraphIndexInfo<int64_t *, SelectionVector> &rai_info,
      relgo::PushDownStatistics &rai_stat);
  void FillBitmaskWithAListIncremental(
      Vector &key_vector, idx_t count,
      relgo::GraphIndexInfo<int64_t *, SelectionVector> &rai_info,
      shared_ptr<relgo::bitmask_vector> row_bitmask_pre,
      relgo::PushDownStatistics &rai_stat);
  void FillBitmaskWithoutAListIncremental(
      Vector &key_vector, idx_t count,
      relgo::GraphIndexInfo<int64_t *, SelectionVector> &rai_info,
      shared_ptr<relgo::bitmask_vector> row_bitmask_pre,
      relgo::PushDownStatistics &rai_stat);
...
public:
  ...
  //! ProbeSpill represents materialized probe-side data that could not be
  //! probed during PhysicalHashJoin::Execute because the HashTable did not fit
  //! in memory. The ProbeSpill is not partitioned if the remaining data can be
  //! dealt with in just 1 more round of probing, otherwise it is radix
  //! partitioned in the same way as the HashTable
  struct SIPProbeSpill {
  public:
    SIPProbeSpill(SIPHashTable &ht, ClientContext &context,
  ...
  private:
    ...
    vector<unique_ptr<PartitionedColumnDataAppendState>>
  ...
  bool RequiresExternalJoin(ClientConfig &config,
                            vector<unique_ptr<SIPHashTable>> &local_hts);
  //! Computes partition sizes and number of radix bits (called before
  //! scheduling partition tasks)
  bool RequiresPartitioning(ClientConfig &config,
                            vector<unique_ptr<SIPHashTable>> &local_hts);
  //! Partition this HT
  ...
  unique_ptr<SIPScanStructure>
  ProbeAndSpill(DataChunk &keys, DataChunk &payload, SIPProbeSpill &probe_spill,
