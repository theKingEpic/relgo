...
class LogicalIndexedGet : public LogicalOperator {
public:
	static constexpr const LogicalOperatorType TYPE = LogicalOperatorType::LOGICAL_GET;

public:
	LogicalIndexedGet(idx_t table_index, IndexedTableFunction function, unique_ptr<FunctionData> bind_data,
	...
	//! Currently stores File Filters (as strings) applied by hive
	//! partitioning/complex filter pushdown Stored so the can be included in
	//! explain output
