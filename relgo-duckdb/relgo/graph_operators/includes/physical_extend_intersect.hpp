...
class PhysicalExtendIntersect : public PhysicalJoin,
...
public:
  PhysicalExtendIntersect(
      LogicalOperator &op, unique_ptr<PhysicalOperator> point,
      unique_ptr<PhysicalOperator> child, vector<IndexedJoinCondition> cond,
      vector<IndexedJoinCondition> other_conditions_p, JoinType join_type,
  ...
  void GetVertexes(relgo::GraphIndex<int64_t *, SelectionVector> &rai,
                   ...
                   bool forward) const;
  void GetVertexesMerge(
      relgo::GraphIndex<int64_t *, SelectionVector> &rai,
      DataChunk &right_chunk, DataChunk &rid_chunk, DataChunk &new_chunk,
      std::vector<idx_t> &left_tuple, idx_t &right_tuple,
  ...
  vector<IndexedJoinCondition> other_conditions;
  //!
  vector<relgo::GraphIndexInfo<int64_t *, SelectionVector> *> merge_rais;
  //!
  std::vector<relgo::CompactList *> compact_lists;
