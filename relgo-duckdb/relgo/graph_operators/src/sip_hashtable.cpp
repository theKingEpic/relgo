...
SIPHashTable::SIPHashTable(BufferManager &buffer_manager_p,
                           const vector<IndexedJoinCondition> &conditions_p,
                           vector<LogicalType> btypes, JoinType type_p)
    : buffer_manager(buffer_manager_p), conditions(conditions_p),
      other_conditions(conditions_p), build_types(std::move(btypes)),
      entry_size(0), tuple_size(0), vfound(Value::BOOLEAN(false)),
      join_type(type_p), finalized(false), has_null(false), external(false),
      radix_bits(4), partition_start(0), partition_end(0) {
  for (auto &condition : conditions) {
    ...
    null_values_are_equal.push_back(
        condition.comparison == ExpressionType::COMPARE_DISTINCT_FROM ||
        condition.comparison == ExpressionType::COMPARE_NOT_DISTINCT_FROM);
  ...
  layout_types.insert(layout_types.end(), build_types.begin(),
                      build_types.end());
  if (IsRightOuterJoin(join_type)) {
    // full/right outer joins need an extra bool to keep track of whether or not
    // a tuple has found a matching entry we place the bool before the NEXT
    // pointer
  ...
  sink_collection = make_uniq<RadixPartitionedTupleData>(
      buffer_manager, layout, radix_bits, layout.ColumnCount() - 1);
}

SIPHashTable::SIPHashTable(
    BufferManager &buffer_manager_p,
    const vector<IndexedJoinCondition> &conditions_p,
    const vector<IndexedJoinCondition> &other_conditions_p,
    vector<LogicalType> btypes, JoinType type_p)
    : buffer_manager(buffer_manager_p), conditions(conditions_p),
      other_conditions(other_conditions_p), build_types(std::move(btypes)),
      entry_size(0), tuple_size(0), vfound(Value::BOOLEAN(false)),
      join_type(type_p), finalized(false), has_null(false), external(false),
      radix_bits(4), partition_start(0), partition_end(0) {
  for (auto &condition : conditions) {
...
void SIPHashTable::ApplyBitmask(Vector &hashes, const SelectionVector &sel,
...
void SIPHashTable::ApplyKeymask(Vector &hashes, const SelectionVector &sel,
                                idx_t count, Vector &pointers) {
  UnifiedVectorFormat hdata;
  hashes.ToUnifiedFormat(count, hdata);
...
void SIPHashTable::Hash(DataChunk &keys, const SelectionVector &sel,
                        idx_t count, Vector &hashes) {
  if (count == keys.size()) {
...
static idx_t SIPFilterNullValues(UnifiedVectorFormat &vdata,
...
idx_t SIPHashTable::PrepareKeys(
    DataChunk &keys, unsafe_unique_array<UnifiedVectorFormat> &key_data,
  ...
  if (build_side && IsRightOuterJoin(join_type)) {
    // in case of a right or full outer join, we cannot remove NULL keys from
    // the build side
    return added_count;
  }
  for (idx_t i = 0; i < keys.ColumnCount(); i++) {
    if (!null_values_are_equal[i]) {
      ...
      added_count =
          SIPFilterNullValues(key_data[i], *current_sel, added_count, sel);
...
void SIPHashTable::Build(PartitionedTupleDataAppendState &append_state,
  ...
  if (join_type == JoinType::MARK &&
      !correlated_mark_join_info.correlated_types.empty()) {
    auto &info = correlated_mark_join_info;
    lock_guard<mutex> mj_lock(info.mj_lock);
    // Correlated MARK join
    // for the correlated mark join we need to keep track of COUNT(*) and
    // COUNT(COLUMN) for each of the correlated columns push into the aggregate
    // hash table
    ...
    info.correlated_payload.data[0].Reference(
        keys.data[info.correlated_types.size()]);
  ...
  // build a chunk to append to the data collection [keys, payload, (optional
  // "found" boolean), hash]
...
static inline void
SIPInsertHashesLoop(atomic<data_ptr_t> pointers[], const hash_t indices[],
                    const idx_t count, const data_ptr_t key_locations[],
  ...
  for (idx_t i = 0; i < count; i++) {
    ...
    if (PARALLEL) {
      ...
      } while (!std::atomic_compare_exchange_weak(&pointers[index], &head,
                                                  key_locations[i]));
    } else {
...
void SIPHashTable::InsertHashes(Vector &hashes, idx_t count,
                                data_ptr_t key_locations[], bool parallel) {
  ...
  if (parallel) {
    SIPInsertHashesLoop<true>(pointers, indices, count, key_locations,
                              pointer_offset);
  } else {
    SIPInsertHashesLoop<false>(pointers, indices, count, key_locations,
                               pointer_offset);
  }
}
...
void SIPHashTable::InitializePointerTable() {
  ...
  if (hash_map.get()) {
    ...
    if (capacity > current_capacity) {
      // Need more space
      hash_map = buffer_manager.GetBufferAllocator().Allocate(
          capacity * sizeof(data_ptr_t));
    } else {
    ...
    // Allocate a hash map
    hash_map = buffer_manager.GetBufferAllocator().Allocate(capacity *
                                                            sizeof(data_ptr_t));
  }
  D_ASSERT(hash_map.GetSize() == capacity * sizeof(data_ptr_t));

  // initialize HT with all-zero entries
  std::fill_n(reinterpret_cast<data_ptr_t *>(hash_map.get()), capacity,
              nullptr);

  bitmask = capacity - 1;
}

void SIPHashTable::Finalize(idx_t chunk_idx_from, idx_t chunk_idx_to,
                            bool parallel) {
  ...
  TupleDataChunkIterator iterator(
      *data_collection, TupleDataPinProperties::KEEP_EVERYTHING_PINNED,
...
unique_ptr<SIPScanStructure>
SIPHashTable::InitializeScanStructure(DataChunk &keys,
                                      const SelectionVector *&current_sel) {
  ...
  ss->count =
      PrepareKeys(keys, ss->key_data, current_sel, ss->sel_vector, false);
  return ss;
}

void SIPHashTable::GenerateBitmaskFilter(
    relgo::GraphIndexInfo<int64_t *, SelectionVector> &rai_info,
  ...
  if (rai_info.passing_tables[1] != 0) {
    auto extra_zone_size =
        (rai_info.left_cardinalities[1] / STANDARD_VECTOR_SIZE) + 1;
    rai_stat.extra_row_bitmask = make_uniq<relgo::bitmask_vector>(
        extra_zone_size * STANDARD_VECTOR_SIZE);
    rai_stat.extra_zone_bitmask =
        make_uniq<relgo::bitmask_vector>(extra_zone_size);
  }
  ...
  if (use_alist) {
    ...
    do {
      const auto count = iterator.GetCurrentChunkCount();
      for (idx_t i = 0; i < count; ++i) {
        key_data[i] = Load<int64_t>(
            (data_ptr_t)(row_locations[i] + layout.GetOffsets()[0]));
      }
      FillBitmaskWithAList(keys, count, rai_info, rai_stat);
    } while (iterator.Next());
  } else {
    idx_t chunk_idx_from = 0;
    idx_t chunk_idx_to = data_collection->ChunkCount();
    ...
    do {
      ...
      FillBitmaskWithoutAList(keys, count, rai_info, rai_stat);
    } while (iterator.Next());
  }
}

void SIPHashTable::GenerateBitmaskFilterIncremental(
    relgo::GraphIndexInfo<int64_t *, SelectionVector> &rai_info,
    relgo::PushDownStatistics &rai_stat_pre,
    relgo::PushDownStatistics &rai_stat, bool use_alist, idx_t key_index) {
  // initialize bitmask filters
  auto zone_size = (rai_info.left_cardinalities[0] / STANDARD_VECTOR_SIZE) + 1;
  rai_stat.row_bitmask =
      make_uniq<relgo::bitmask_vector>(zone_size * STANDARD_VECTOR_SIZE);
  ...
  if (use_alist) {
    ...
    do {
      ...
      for (idx_t i = 0; i < count; ++i) {
        key_data[i] = Load<int64_t>(
            (data_ptr_t)(row_locations[i] + layout.GetOffsets()[key_index]));
      }
      FillBitmaskWithAListIncremental(keys, count, rai_info,
                                      rai_stat_pre.row_bitmask, rai_stat);
    } while (iterator.Next());
  } else {
    idx_t chunk_idx_from = 0;
    idx_t chunk_idx_to = data_collection->ChunkCount();
    ...
    do {
      ...
      FillBitmaskWithoutAListIncremental(keys, count, rai_info,
                                         rai_stat_pre.row_bitmask, rai_stat);
    } while (iterator.Next());
  }
}

void SIPHashTable::FillBitmaskWithAList(
    Vector &key_vector, idx_t count,
    relgo::GraphIndexInfo<int64_t *, SelectionVector> &rai_info,
    relgo::PushDownStatistics &rai_stat) {
  ...
  auto &row_bitmask = *rai_stat.row_bitmask;
  auto &zone_bitmask = *rai_stat.zone_bitmask;
  ...
  if (rai_info.passing_tables[1] == 0) {
    ...
    auto &extra_zone_filter = *rai_stat.extra_row_bitmask;
    auto &extra_zone_sel = *rai_stat.extra_zone_bitmask;
    for (idx_t i = 0; i < count; i++) {
      idx_t pos = key_data.sel->get_index(i);
      auto offset = compact_list->offsets[keys[pos]];
      auto size = compact_list->sizes[keys[pos]];
...
void SIPHashTable::FillBitmaskWithoutAList(
    Vector &key_vector, idx_t count,
    relgo::GraphIndexInfo<int64_t *, SelectionVector> &rai_info,
    relgo::PushDownStatistics &rai_stat) {
  UnifiedVectorFormat key_data;
...
void SIPHashTable::FillBitmaskWithAListIncremental(
    Vector &key_vector, idx_t count,
    relgo::GraphIndexInfo<int64_t *, SelectionVector> &rai_info,
    shared_ptr<relgo::bitmask_vector> row_bitmask_pre,
    relgo::PushDownStatistics &rai_stat) {
  UnifiedVectorFormat key_data;
  key_vector.ToUnifiedFormat(count, key_data);
  auto *keys = UnifiedVectorFormat::GetData<int64_t>(key_data);
...
void SIPHashTable::FillBitmaskWithoutAListIncremental(
    Vector &key_vector, idx_t count,
    relgo::GraphIndexInfo<int64_t *, SelectionVector> &rai_info,
    shared_ptr<relgo::bitmask_vector> row_bitmask_pre,
    relgo::PushDownStatistics &rai_stat) {
...
SIPScanStructure::SIPScanStructure(SIPHashTable &ht)
    : pointers(LogicalType::POINTER), sel_vector(STANDARD_VECTOR_SIZE), ht(ht),
      finished(false) {}

void SIPScanStructure::Next(DataChunk &keys, DataChunk &left,
                            DataChunk &result) {
  if (finished) {
    return;
  }
...
idx_t SIPScanStructure::ResolvePredicates(DataChunk &keys,
  ...
  return RowOperations::Match(keys, key_data.get(), ht.layout, pointers,
                              ht.predicates, match_sel, this->count,
                              no_match_sel, no_match_count);
}

idx_t SIPScanStructure::ScanInnerJoin(DataChunk &keys,
...
void SIPScanStructure::AdvancePointers(const SelectionVector &sel,
                                       idx_t sel_count) {
...
void SIPScanStructure::InitializeSelectionVector(
    const SelectionVector *&current_sel) {
...
void SIPScanStructure::GatherResult(Vector &result,
                                    const SelectionVector &result_vector,
                                    const SelectionVector &sel_vector,
                                    const idx_t count, const idx_t col_no) {
  ht.data_collection->Gather(pointers, sel_vector, count, col_no, result,
                             result_vector);
}
...
void SIPScanStructure::GatherResult(Vector &result,
  ...
  GatherResult(result, *FlatVector::IncrementalSelectionVector(), sel_vector,
               count, col_idx);
}

void SIPScanStructure::NextInnerJoin(DataChunk &keys, DataChunk &left,
                                     DataChunk &result) {
  ...
  if (result_count > 0) {
    if (IsRightOuterJoin(ht.join_type)) {
      ...
      for (idx_t i = 0; i < result_count; i++) {
        auto idx = result_vector.get_index(i);
        // NOTE: threadsan reports this as a data race because this can be set
        // concurrently by separate threads Technically it is, but it does not
        // matter, since the only value that can be written is "true"
...
void SIPScanStructure::ScanKeyMatches(DataChunk &keys) {
  // the semi-join, anti-join and mark-join we handle a differently from the
  // inner join since there can be at most STANDARD_VECTOR_SIZE results we
  // handle the entire chunk in one call to Next(). for every pointer, we keep
  // chasing pointers and doing comparisons. this results in a boolean array
  // indicating whether or not the tuple has a match
  SelectionVector match_sel(STANDARD_VECTOR_SIZE),
      no_match_sel(STANDARD_VECTOR_SIZE);
  while (this->count > 0) {
...
template <bool MATCH>
void SIPScanStructure::NextSemiOrAntiJoin(DataChunk &keys, DataChunk &left,
                                          DataChunk &result) {
...
void SIPScanStructure::NextSemiJoin(DataChunk &keys, DataChunk &left,
                                    DataChunk &result) {
...
void SIPScanStructure::NextAntiJoin(DataChunk &keys, DataChunk &left,
                                    DataChunk &result) {
  // first scan for key matches
  ScanKeyMatches(keys);
...
void SIPScanStructure::ConstructMarkJoinResult(DataChunk &join_keys,
                                               DataChunk &child,
                                               DataChunk &result) {
  ...
  // now set the remaining entries to either true or false based on whether a
  // match was found
  if (found_match) {
  ...
  // if the right side contains NULL values, the result of any FALSE becomes
  // NULL
  if (ht.has_null) {
    for (idx_t i = 0; i < child.size(); i++) {
      if (!bool_result[i]) {
...
void SIPScanStructure::NextMarkJoin(DataChunk &keys, DataChunk &input,
                                    DataChunk &result) {
  ...
  if (ht.correlated_mark_join_info.correlated_types.empty()) {
    ...
    // first we fetch the counts from the aggregate hashtable corresponding to
    // these entries
    ...
    info.correlated_counts->FetchAggregates(info.group_chunk,
                                            info.result_chunk);
    ...
    // first set the nullmask based on whether or not there were NULL values in
    // the join key
    ...
    // set the entries to either true or false based on whether a match was
    // found
    for (idx_t i = 0; i < input.size(); i++) {
...
void SIPScanStructure::NextLeftJoin(DataChunk &keys, DataChunk &left,
                                    DataChunk &result) {
...
void SIPScanStructure::NextSingleJoin(DataChunk &keys, DataChunk &input,
                                      DataChunk &result) {
  // single join
  ...
  for (idx_t i = 0; i < ht.build_types.size(); i++) {
    ...
    GatherResult(vector, result_sel, result_sel, result_count,
                 i + ht.condition_types.size());
  }
  result.SetCardinality(input.size());

  // like the SEMI, ANTI and MARK join types, the SINGLE join only ever does one
  // pass over the HT per input chunk
  finished = true;
}

void SIPHashTable::ScanFullOuter(SIPHTScanState &state, Vector &addresses,
                                 DataChunk &result) {
  // scan the HT starting from the current position and check which rows from
  // the build side did not find a match
  ...
  for (idx_t i = 0; i < build_types.size(); i++) {
    ...
    data_collection->Gather(addresses, sel_vector, found_entries, col_no,
                            vector, sel_vector);
  }
}

idx_t SIPHashTable::FillWithHTOffsets(SIPHTScanState &state,
                                      Vector &addresses) {
  // iterate over HT
  auto key_locations = FlatVector::GetData<data_ptr_t>(addresses);
  idx_t key_count = 0;
...
bool SIPHashTable::RequiresExternalJoin(
    ClientConfig &config, vector<unique_ptr<SIPHashTable>> &local_hts) {
  total_count = 0;
  idx_t data_size = 0;
...
bool SIPHashTable::RequiresPartitioning(
    ClientConfig &config, vector<unique_ptr<SIPHashTable>> &local_hts) {
  ...
  for (auto &ht : local_hts) {
    ...
    for (idx_t partition_idx = 0; partition_idx < num_partitions;
         partition_idx++) {
  ...
  if (config.force_external || max_partition_size > max_ht_size) {
    ...
    for (; added_bits < max_added_bits; added_bits++) {
      double partition_multiplier =
          RadixPartitioning::NumberOfPartitions(added_bits);
      ...
      auto new_estimated_ht_size =
          new_estimated_size + PointerTableSize(new_estimated_count);

      if (config.force_external ||
          new_estimated_ht_size <= double(max_ht_size) / 4) {
...
bool SIPHashTable::PrepareExternalFinalize() {
  ...
  idx_t data_size = 0;
  idx_t partition_idx;
  for (partition_idx = partition_start; partition_idx < num_partitions;
       partition_idx++) {
  ...
  for (partition_idx = partition_start; partition_idx < partition_end;
       partition_idx++) {
...
static void SIPCreateSpillChunk(DataChunk &spill_chunk, DataChunk &keys,
                                DataChunk &payload, Vector &hashes) {
  spill_chunk.Reset();
...
unique_ptr<SIPScanStructure> SIPHashTable::ProbeAndSpill(
    DataChunk &keys, DataChunk &payload, SIPProbeSpill &probe_spill,
  ...
  auto true_count = RadixPartitioning::Select(
      hashes, FlatVector::IncrementalSelectionVector(), keys.size(), radix_bits,
...
SIPProbeSpill::SIPProbeSpill(SIPHashTable &ht, ClientContext &context,
  ...
  auto remaining_ht_size =
      remaining_data_size + ht.PointerTableSize(remaining_count);
  if (remaining_ht_size <= ht.max_ht_size) {
    ...
    global_partitions = make_uniq<RadixPartitionedColumnData>(
        context, probe_types, ht.radix_bits, probe_types.size() - 1);
  }
...
SIPProbeSpillLocalState SIPProbeSpill::RegisterThread() {
  ...
  if (partitioned) {
    ...
    local_partition_append_states.emplace_back(
        make_uniq<PartitionedColumnDataAppendState>());
    local_partitions.back()->InitializeAppendState(
        *local_partition_append_states.back());

    result.local_partition = local_partitions.back().get();
    result.local_partition_append_state =
        local_partition_append_states.back().get();
  } else {
    local_spill_collections.emplace_back(make_uniq<ColumnDataCollection>(
        BufferManager::GetBufferManager(context), probe_types));
    local_spill_append_states.emplace_back(make_uniq<ColumnDataAppendState>());
    local_spill_collections.back()->InitializeAppend(
        *local_spill_append_states.back());
...
void SIPProbeSpill::Append(DataChunk &chunk,
                           SIPProbeSpillLocalAppendState &local_state) {
  if (partitioned) {
    local_state.local_partition->Append(
        *local_state.local_partition_append_state, chunk);
  } else {
    local_state.local_spill_collection->Append(
        *local_state.local_spill_append_state, chunk);
  }
}
...
void SIPProbeSpill::Finalize() {
  if (partitioned) {
    ...
    if (local_spill_collections.empty()) {
      global_spill_collection = make_uniq<ColumnDataCollection>(
          BufferManager::GetBufferManager(context), probe_types);
    } else {
...
void SIPProbeSpill::PrepareNextProbe() {
  ...
  consumer =
      make_uniq<ColumnDataConsumer>(*global_spill_collection, column_ids);
