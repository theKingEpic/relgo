#include "../includes/logical_indexed_get.hpp"
...
LogicalIndexedGet::LogicalIndexedGet() : LogicalOperator(LogicalOperatorType::LOGICAL_GET) {
}

LogicalIndexedGet::LogicalIndexedGet(idx_t table_index, IndexedTableFunction function,
...
optional_ptr<TableCatalogEntry> LogicalIndexedGet::GetTable() const {
	return IndexedTableScanFunction::GetTableEntry(function, bind_data.get());
}

string LogicalIndexedGet::ParamsToString() const {
	string result = "";
...
vector<ColumnBinding> LogicalIndexedGet::GetColumnBindings() {
	if (column_ids.empty()) {
	...
	if (!projected_input.empty()) {
		if (children.size() != 1) {
			throw InternalException("LogicalGet::project_input can only be set for "
...
void LogicalIndexedGet::ResolveTypes() {
	if (column_ids.empty()) {
	...
	if (projection_ids.empty()) {
		for (auto &index : column_ids) {
			...
			} else if (index >= relgo::COLUMN_IDENTIFIER_GRAPH_INDEX_ID) {
				types.emplace_back(LogicalType::ROW_TYPE);
			} else {
...
idx_t LogicalIndexedGet::EstimateCardinality(ClientContext &context) {
...
void LogicalIndexedGet::Serialize(Serializer &serializer) const {
	...
	if (!function.serialize) {
		...
		// no serialize method: serialize input values and named_parameters for
		// rebinding purposes
...
unique_ptr<LogicalOperator> LogicalIndexedGet::Deserialize(Deserializer &deserializer) {
	auto result = unique_ptr<LogicalIndexedGet>(new LogicalIndexedGet());
	...
	//   auto entry = nullptr;
	//   FunctionSerializer::DeserializeBase<IndexedTableFunction,
	...
	if (!has_serialize) {
		...
		if (result->returned_types != bind_return_types) {
			throw SerializationException("Table function deserialization failure - bind returned different "
			                             "return types than were serialized");
		}
		// names can actually be different because of aliases - only the sizes
		// cannot be different
		if (result->names.size() != bind_names.size()) {
			throw SerializationException("Table function deserialization failure - bind returned different "
			                             "returned names than were serialized");
		}
	} else {
		// bind_data = FunctionSerializer::FunctionDeserialize(deserializer,
		// function);
	}
...
vector<idx_t> LogicalIndexedGet::GetTableIndex() const {
	return vector<idx_t> {table_index};
}

string LogicalIndexedGet::GetName() const {
#ifdef DEBUG
