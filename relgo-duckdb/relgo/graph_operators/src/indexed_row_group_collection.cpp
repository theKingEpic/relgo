...
//===--------------------------------------------------------------------===//
IndexedRowGroupSegmentTree::IndexedRowGroupSegmentTree(IndexedRowGroupCollection &collection)
    : SegmentTree<IndexedRowGroup, true>(), collection(collection), current_row_group(0), max_row_group(0) {
}
IndexedRowGroupSegmentTree::~IndexedRowGroupSegmentTree() {
}

void IndexedRowGroupSegmentTree::Initialize(PersistentTableData &data) {
...
unique_ptr<IndexedRowGroup> IndexedRowGroupSegmentTree::IndexedLoadSegment() {
	...
	deserializer.End();
	current_row_group++;
	return make_uniq<IndexedRowGroup>(collection, std::move(row_group_pointer));
}

//===--------------------------------------------------------------------===//
// Row Group Collection
//===--------------------------------------------------------------------===//
IndexedRowGroupCollection::IndexedRowGroupCollection(shared_ptr<DataTableInfo> info_p, BlockManager &block_manager,
	...
	row_groups = make_shared<IndexedRowGroupSegmentTree>(*this);
}

IndexedRowGroupCollection::IndexedRowGroupCollection(RowGroupCollection &rgc)
    : block_manager(rgc.GetBlockManager()), total_rows(rgc.GetTotalRows()), types(rgc.GetTypes()) {
	ParallelCollectionScanState parallel_state;
	rgc.InitializeParallelScan(parallel_state);
	row_start = parallel_state.max_row - rgc.GetTotalRows();

	auto &rgc_info = rgc.GetTableInfo();
	info = make_shared<DataTableInfo>(rgc_info.db, rgc_info.table_io_manager, rgc_info.schema, rgc_info.table);
	row_groups = make_shared<IndexedRowGroupSegmentTree>(*this);

	RowGroup *row_group = rgc.GetRowGroup(0);
	while (row_group) {
		unique_ptr<IndexedRowGroup> indexed_rg = make_uniq<IndexedRowGroup>(*this, *row_group);
		row_groups->AppendSegment(move(indexed_rg));
		row_group = row_group->Next();
	}
}

idx_t IndexedRowGroupCollection::GetTotalRows() const {
	return total_rows.load();
}

const vector<LogicalType> &IndexedRowGroupCollection::GetTypes() const {
	return types;
}

Allocator &IndexedRowGroupCollection::GetAllocator() const {
	return Allocator::Get(info->db);
}

AttachedDatabase &IndexedRowGroupCollection::GetAttached() {
	return GetTableInfo().db;
}

MetadataManager &IndexedRowGroupCollection::GetMetadataManager() {
...
// Initialize
//===--------------------------------------------------------------------===//
void IndexedRowGroupCollection::Initialize(PersistentTableData &data) {
...
void IndexedRowGroupCollection::InitializeEmpty() {
	stats.InitializeEmpty(types);
}

void IndexedRowGroupCollection::AppendRowGroup(SegmentLock &l, idx_t start_row) {
	D_ASSERT(start_row >= row_start);
	auto new_row_group = make_uniq<IndexedRowGroup>(*this, start_row, 0);
	new_row_group->InitializeEmpty(types);
	row_groups->AppendSegment(l, std::move(new_row_group));
}

IndexedRowGroup *IndexedRowGroupCollection::GetRowGroup(int64_t index) {
	return (IndexedRowGroup *)row_groups->GetSegmentByIndex(index);
}

void IndexedRowGroupCollection::Verify() {
#ifdef DEBUG
...
// Scan
//===--------------------------------------------------------------------===//
void IndexedRowGroupCollection::InitializeScan(IndexedCollectionScanState &state, const vector<column_t> &column_ids,
...
void IndexedRowGroupCollection::InitializeCreateIndexScan(IndexedCreateIndexScanState &state) {
	state.segment_lock = row_groups->Lock();
}

void IndexedRowGroupCollection::InitializeScanWithOffset(IndexedCollectionScanState &state,
                                                         const vector<column_t> &column_ids, idx_t start_row,
                                                         idx_t end_row) {
...
bool IndexedRowGroupCollection::InitializeScanInRowGroup(IndexedCollectionScanState &state,
                                                         IndexedRowGroupCollection &collection,
                                                         IndexedRowGroup &row_group, idx_t vector_index,
                                                         idx_t max_row) {
...
bool IndexedRowGroupCollection::Scan(DuckTransaction &transaction, const vector<column_t> &column_ids,
...
bool IndexedRowGroupCollection::Scan(DuckTransaction &transaction, const std::function<bool(DataChunk &chunk)> &fun) {
...
// Fetch
//===--------------------------------------------------------------------===//
void IndexedRowGroupCollection::Fetch(TransactionData transaction, DataChunk &result,
...
// Append
//===--------------------------------------------------------------------===//
IndexedTableAppendState::IndexedTableAppendState()
...
bool IndexedRowGroupCollection::IsEmpty() const {
	auto l = row_groups->Lock();
	return IsEmpty(l);
}

bool IndexedRowGroupCollection::IsEmpty(SegmentLock &l) const {
	return row_groups->IsEmpty(l);
}

void IndexedRowGroupCollection::InitializeAppend(TransactionData transaction, IndexedTableAppendState &state,
                                                 idx_t append_count) {
...
void IndexedRowGroupCollection::InitializeAppend(IndexedTableAppendState &state) {
	TransactionData tdata(0, 0);
	InitializeAppend(tdata, state, 0);
}

bool IndexedRowGroupCollection::Append(DataChunk &chunk, IndexedTableAppendState &state) {
	...
	while (true) {
		...
		if (remaining > 0) {
			// we expect max 1 iteration of this loop (i.e. a single chunk should
			// never overflow more than one row_group)
...
void IndexedRowGroupCollection::FinalizeAppend(TransactionData transaction, IndexedTableAppendState &state) {
...
void IndexedRowGroupCollection::CommitAppend(transaction_t commit_id, idx_t row_start, idx_t count) {
...
void IndexedRowGroupCollection::RevertAppendInternal(idx_t start_row, idx_t count) {
...
void IndexedRowGroupCollection::MergeStorage(IndexedRowGroupCollection &data) {
	if (data.types != types) {
		int to_stop = 0;
	}
...
// Delete
//===--------------------------------------------------------------------===//
idx_t IndexedRowGroupCollection::Delete(TransactionData transaction, DataTable &table, row_t *ids, idx_t count) {
	...
	// we iterate over the ids and check for every id if it belongs to the same
	// row group as their predecessor
	idx_t pos = 0;
	do {
		idx_t start = pos;
...
// Update
//===--------------------------------------------------------------------===//
void IndexedRowGroupCollection::Update(TransactionData transaction, row_t *ids, const vector<PhysicalIndex> &column_ids,
                                       DataChunk &updates) {
	idx_t pos = 0;
	do {
		idx_t start = pos;
...
void IndexedRowGroupCollection::RemoveFromIndexes(TableIndexList &indexes, Vector &row_identifiers, idx_t count) {
	auto row_ids = FlatVector::GetData<row_t>(row_identifiers);

	// initialize the fetch state
	// FIXME: we do not need to fetch all columns, only the columns required by
	// the indices!
	IndexedTableScanState state;
	vector<column_t> column_ids;
	column_ids.reserve(types.size());
	...
	for (idx_t r = 0; r < count;) {
		...
		// slice the vector with all rows that are present in this vector and erase
		// from the index
...
void IndexedRowGroupCollection::UpdateColumn(TransactionData transaction, Vector &row_ids,
...
// Checkpoint
//===--------------------------------------------------------------------===//
void IndexedRowGroupCollection::Checkpoint(TableDataWriter &writer, TableStatistics &global_stats) {
...
// CommitDrop
//===--------------------------------------------------------------------===//
void IndexedRowGroupCollection::CommitDropColumn(idx_t index) {
	for (auto &row_group : row_groups->Segments()) {
		row_group.CommitDropColumn(index);
	}
}

void IndexedRowGroupCollection::CommitDropTable() {
	for (auto &row_group : row_groups->Segments()) {
		row_group.CommitDrop();
	}
}
...
//===--------------------------------------------------------------------===//
vector<ColumnSegmentInfo> IndexedRowGroupCollection::GetColumnSegmentInfo() {
...
IndexedRowGroupCollection::AddColumn(ClientContext &context, ColumnDefinition &new_column, Expression &default_value) {
	idx_t new_column_idx = types.size();
	auto new_types = types;
	new_types.push_back(new_column.GetType());
	auto result =
	    make_shared<IndexedRowGroupCollection>(info, block_manager, std::move(new_types), row_start, total_rows.load());
...
shared_ptr<IndexedRowGroupCollection> IndexedRowGroupCollection::RemoveColumn(idx_t col_idx) {
...
shared_ptr<IndexedRowGroupCollection> IndexedRowGroupCollection::AlterType(ClientContext &context, idx_t changed_idx,
...
void IndexedRowGroupCollection::VerifyNewConstraint(DataTable &parent, const BoundConstraint &constraint) {
	if (total_rows == 0) {
		return;
	}
	...
	IndexedCreateIndexScanState state;
...
void IndexedRowGroupCollection::InitializeParallelScan(IndexedParallelCollectionScanState &state) {
...
bool IndexedRowGroupCollection::NextParallelScan(ClientContext &context, IndexedParallelCollectionScanState &state,
...
// Statistics
//===--------------------------------------------------------------------===//
void IndexedRowGroupCollection::CopyStats(TableStatistics &other_stats) {
	stats.CopyStats(other_stats);
}

unique_ptr<BaseStatistics> IndexedRowGroupCollection::CopyStats(column_t column_id) {
	return stats.CopyStats(column_id);
}

void IndexedRowGroupCollection::SetDistinct(column_t column_id, unique_ptr<DistinctStatistics> distinct_stats) {
